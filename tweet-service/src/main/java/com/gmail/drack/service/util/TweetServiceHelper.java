...
import com.gmail.drack.broker.producer.TagProducer;
import com.gmail.drack.broker.producer.TweetNotificationProducer;
import com.gmail.drack.broker.producer.UpdateTweetCountProducer;
...
public class TweetServiceHelper {
    ...
    private void sendUserReplyMention(Tweet tweet, User authUser) {
        if (tweet.getAddressedUser() != null) {
            userService.getUserById(tweet.getAddressedUser().getId())
                    .ifPresent(user -> tweetNotificationProducer
                            .sendTweetMentionNotificationEvent(NotificationType.REPLY, tweet, user, authUser));
        }
    }
    ...
    private void sendUserMentions(Tweet tweet, User authUser) {
        parseUserMentionFromText(tweet.getText()).forEach(username -> userService.getUserIdByUsername(username)
                .ifPresent(user -> tweetNotificationProducer.sendTweetMentionNotificationEvent(NotificationType.MENTION,
                        tweet, user, authUser)));
    }
